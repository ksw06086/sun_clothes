...
<%@ include file = "topmanu.jsp" %>
    ...
    <div id = "golist" style = "margin: 10px 0px 50px; text-align: right;">
    	<a href = "notice.do?num=${num}&number=${number}&pageNum=${pageNum}&choose=${choose}">
    	...
    	<table>
    		...
    		<c:if test="${vo.fwsubject != null}">
	    		<tr id = "before">
	    			...
	    			<td><a href = "noticeForm.do?num=${vo.fwnum}&number=${number-1}&pageNum=${pageNum}&choose=${choose}">${vo.fwsubject}</a></td>
	    		</tr>
    		</c:if>
    		<c:if test="${vo.nextsubject != null}">
	    		<tr id = "after">
	    			<td><img src = "ascloimage/down.png" width = "12px" height = "12px">다음글</td>
	    			<td><a href = "noticeForm.do?num=${vo.nextnum}&number=${number+1}&pageNum=${pageNum}&choose=${choose}">${vo.nextsubject}</a></td>
	    		</tr>
    		</c:if>
    	</table>
    </div>
