...
<link type = "text/css" rel = "stylesheet" href = "${project}cssall/h_notice.css"/>
<html>
<body onload = "BaseSize('${pageSize}');">
			...
			<td id = "tabright">
				...
				<form action = "h_noticedeletePro.do" method = "post" name = "completeForm">
					...
					<table id = "faqlist">
				    	...
				    	<col style = "width:10%;">
				    	<tr id = "title">
				    		<td><input type = "checkbox" name = "prdcheck" id = "noticeallcheck"  value = "전체선택"></td>
				    		<td>번호</td>
				    		<td>제목</td>
				    		<td>작성자</td>
				    		<td>작성일</td>
						...
						<c:if test="${srhCnt > 0}">
							<c:forEach var = "list" items = "${list}">
								<tr style = "text-align:center;" class = "faq">
									<td><input type = "checkbox" name = "noticechecks" class = "noticecheck"  value = "${list.num}"></td>
									<td>${number}(${list.num})
									<c:set var = "number" value = "${number-1}"/>
									</td>
									<!-- 상세 페이지 -->
									<td align = "left">
									<a href = "h_noticeForm.do?num=${list.num}&number=${number+1}&pageNum=${pageNum}&choose=${choose}">
									${list.subject}
									</a></td>
									<td>${list.writer}</td>
										...
										<th align = "center">
											...
											<c:if test="${cnt > 0}">
												...
												<c:if test="${startPage > pageBlock}">
													<a href = "h_notice.do?choose=${choose}">[◀◀]</a>
													<a href = "h_notice.do?pageNum=${startPage - pageBlock}&choose=${choose}">[◀]</a>
												</c:if>
												...
												<c:forEach var = "i" begin = "${startPage}" end = "${endPage}">
													...
													<c:if test="${i != currentPage}">
														<span><a href = "h_notice.do?pageNum=${i}&choose=${choose}">[${i}]</a></span>				
													</c:if>
												</c:forEach>
												...
												<c:if test="${pageCount > endPage}">
													<a href = "h_notice.do?pageNum=${startPage + pageBlock}&choose=${choose}">[▶]</a>
													<a href = "h_notice.do?pageNum=${pageCount}&choose=${choose}">[▶▶]</a>
												</c:if>
											</c:if>
										</th>
									</tr>
